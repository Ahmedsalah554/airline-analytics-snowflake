...
WITH route_stats AS (
    ...
    FROM {{ ref('fct_daily_flight_offers') }} f
    LEFT JOIN {{ ref('stg_routes') }} r ON f.route_id = r.route_id
...
LEFT JOIN {{ ref('stg_airports') }} o ON v.origin_airport_id = o.airport_id
LEFT JOIN {{ ref('stg_airports') }} d ON v.destination_airport_id = d.airport_id
